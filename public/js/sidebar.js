 ...
 *
 * v2 — Suporte a colapso/expansão com persistência via localStorage
 */
const Sidebar = (() => {
  ...
  function render(activeId, usuario) {
    ...
    // SVG seta esquerda (o CSS rotaciona 180° quando collapsed)
    const arrowSVG = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="15 18 9 12 15 6"/></svg>`;

    return `
      <aside class="sidebar" role="navigation" aria-label="Menu principal">
        <!-- Botão toggle — fica na borda direita da sidebar -->
        <button class="sidebar-toggle" id="sidebar-toggle-btn" title="Recolher menu" aria-label="Recolher/expandir menu">
          ${arrowSVG}
        </button>
        ...
        <div class="sidebar-footer">
          <div class="sidebar-user-card" style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:10px;background:var(--surface-2);border:1px solid var(--border);margin-bottom:8px">
            ${avatarHtml}
            <div class="sidebar-user-info" style="flex:1;min-width:0">
              <div class="sidebar-user-name" style="font-size:0.8125rem;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${usuario?.nome||'...'}</div>
              <div class="sidebar-user-role" style="font-size:0.6875rem;color:var(--text-muted)">${ROLE_LABELS[role]||role}</div>
            </div>
          </div>
          <button class="btn btn-ghost btn-sm w-full" id="btn-logout">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
            <span class="btn-logoff-label">Sair</span>
          </button>
          <div id="sidebar-powered" style="margin-top:10px;text-align:center;font-size:0.6rem;color:var(--text-muted);opacity:0.45;letter-spacing:0.03em;line-height:1.4;padding:0 8px;">
  ...
  /** Aplica/remove classe collapsed na sidebar e no body */
  ...
  /** Configura o botão toggle e lê preferência salva */
